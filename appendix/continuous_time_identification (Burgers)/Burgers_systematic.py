...
class PhysicsInformedNN:
    ...
    def __init__(self, X, u, layers, lb, ub):
        ...
        self.loss = tf.reduce_mean(tf.square(self.u_tf - self.u_pred)) + tf.reduce_mean(
...
def main_loop(N_u, noise, num_layers, num_neurons):
    ...
    layers = (
        np.concatenate([[2], num_neurons * np.ones(num_layers), [1]])
        .astype(int)
        .tolist()
    )
...
if __name__ == "__main__":
    ...
    for i in range(len(N_u)):
        for j in range(len(noise)):
            error_lambda_1_table_1[i, j], error_lambda_2_table_1[i, j] = main_loop(
                N_u[i], noise[j], num_layers[-1], num_neurons[-1]
            )

    for i in range(len(num_layers)):
        for j in range(len(num_neurons)):
            error_lambda_1_table_2[i, j], error_lambda_2_table_2[i, j] = main_loop(
                N_u[-1], noise[0], num_layers[i], num_neurons[j]
            )

    np.savetxt(
        "./tables/error_lambda_1_table_1.csv",
        error_lambda_1_table_1,
        delimiter=" & ",
        fmt="$%2.3f$",
        newline=" \\\\\n",
    )
    np.savetxt(
        "./tables/error_lambda_2_table_1.csv",
        error_lambda_2_table_1,
        delimiter=" & ",
        fmt="$%2.3f$",
        newline=" \\\\\n",
    )
    ...
    np.savetxt(
        "./tables/error_lambda_1_table_2.csv",
        error_lambda_1_table_2,
        delimiter=" & ",
        fmt="$%2.3f$",
        newline=" \\\\\n",
    )
    np.savetxt(
        "./tables/error_lambda_2_table_2.csv",
        error_lambda_2_table_2,
        delimiter=" & ",
        fmt="$%2.3f$",
        newline=" \\\\\n",
    )
